...
public interface RepositoryInterface extends JpaRepository<EncryptionBean, Long> {
    ...
    @Query("SELECT s FROM EncryptionBean s WHERE " +
           ...
           "CAST(s.password AS text) LIKE CONCAT('%', :query, '%') OR " +
           "CAST(s.aadhaarNumber AS text) LIKE CONCAT('%', :query, '%') OR " +
           "CAST(s.mobileNumber AS text) LIKE CONCAT('%', :query, '%') OR " +
           "LOWER(s.bankName) LIKE LOWER(CONCAT('%', :query, '%'))")
    List<EncryptionBean> findByKeyword(@Param("query") String query);
}
